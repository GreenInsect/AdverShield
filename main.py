...
APDS_processor = PatchDetector(3, 1, base_filter=16, square_sizes=[150, 100, 75, 50, 25], n_patch=1)
APDS_processor.unet.load_state_dict(torch.load(ckpt_path, map_location=device))
APDS_processor.unet.to(device)
APDS_processor.unet.eval()
...
def detect_and_remove_patch(tensor):
    with torch.no_grad():
        x_processed, _, _ = APDS_processor(tensor, bpda=True, shape_completion=False)
        return x_processed
