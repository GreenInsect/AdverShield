...
class MaxProbExtractor(nn.Module):
    ...
    从 YOLO 输出中提取特定类别的最大类别概率
    FOCUS 继承自 nn.Module 使其成为 PyTorch 计算图的一部分，确保模型输出到损失函数的求导链条不中断
    :Return:
        max_conf: 每张图片中检测到人的最高置信度值
    """
    ...
    def forward(self, YOLOoutput):
        ...
        if YOLOoutput.dim() == 3:
            ...
            """
            YOLOoutput 原为 (C, H, W)，处理后变为 (1, C, H, W)
            确保即使输入单张图片，也能以“批次(Batch)”的形式统一处理
            """
            YOLOoutput = YOLOoutput.unsqueeze(0)
        batch = YOLOoutput.size(0)
        """
        验证通道数是否符合 YOLOv2 格式。5 个 Anchor 框，每个框有 5 个坐标信息（x, y, w, h, objectness）+80 个类别概率。85 * 5 = 425
        YOLOoutput 此时通常是 (Batch, 425, 19, 19)
        在 YOLO 中，图片被划分为 19 * 19 的网格。每一个网格点都要负责预测出一定数量的物体
        """
        assert (YOLOoutput.size(1) == (5 + self.num_cls ) * 5)
        h = YOLOoutput.size(2)
        w = YOLOoutput.size(3)
        # transform the output tensor from [batch, 425, 19, 19] to [batch, 80, 1805]
        output = YOLOoutput.view(batch, 5, 5 + self.num_cls , h * w)  # [batch, 5, 85, 361] == [Batch, 5个Anchor, 85个属性, 361 个网格点]
        # transpose(1, 2) 交换维度；contiguous() 保证内存连续
        output = output.transpose(1, 2).contiguous()  # [batch, 85, 5, 361]
        output = output.view(batch, 5 + self.num_cls , 5 * h * w)  # [batch, 85, 1805] 5 * 19 * 19 == 1805 将空间上的网格展开，方便直接提取所有的 objectness 和类概率
        # torch.sigmoid()。将输出映射到 [0, 1]
        output_objectness = torch.sigmoid(output[:, 4, :])  # [batch, 1805] 代表每个预测框包含“物体”的信心
        output = output[:, 5:5 + self.num_cls , :]  # [batch, 80, 1805]
        # perform softmax to normalize probabilities for object classes to [0,1] Softmax(dim=1)。对 80 个类别进行归一化
        normal_confs = torch.nn.Softmax(dim=1)(output) # normal_confs 是 [Batch, 80, 1805]。代表每个框属于某个类别的条件概率
        # we only care for probabilities of the class of interest (person)
        # confs_for_class 是 [Batch, 1805]。专门提取出“人”这一类的概率
        confs_for_class = normal_confs[:, self.cls_id, :]
        confs_if_object = output_objectness #confs_for_class * output_objectness
        """
        FOCUS 在论文中，作者发现将 objectness（物体存在感）作为目标效果最好。这里调用了配置中的策略函数(指self.config.loss_target)
        """
        confs_if_object = confs_for_class * output_objectness
        # 见 patch_config.py -> class BaseConfig -> self.loss_target = lambda obj, cls: obj * cls
        confs_if_object = self.config.loss_target(output_objectness, confs_for_class)
        # find the max probability for person
        # max_conf 是 [Batch] 长度的向量。记录了每张图中 YOLO 认为最像人的那个框的分数
...
class PatchApplier(nn.Module):
    ...
    def forward(self, img_batch, adv_batch):
        """
        torch.unbind(tensor, dim)。将张量在指定维度上移除，并返回一个元组
        将这“14 层补丁”拆解开，准备逐个“覆盖”到原图上
        """
        advs = torch.unbind(adv_batch, 1)
        for adv in advs:
            """
            这是一个原地更新操作，不断将新的补丁叠加到同一张图上
            """
